...
public interface Validator<T> {
    ...
    default T validateAndDecrypt(final Key key, final Token token) {
        ...
        final byte[] plainText = token.validateAndDecrypt(key); //, now - getTimeToLive(), now + getMaxClockSkew());
     ...
     */
    @SuppressWarnings("PMD.LawOfDemeter")
    default T validateAndDecrypt(final Collection<? extends Key> keys, final Token token) throws Throwable {
        final Key key =
                keys.parallelStream()
